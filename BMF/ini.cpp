...
bmf::INI::File::File (const wchar_t* filename)
{
  ...
  if (ret == 0 && fINI != 0) {
    ...
    //BMF_MessageBox (L"Unable to Locate INI File", filename, MB_OK);
    delete [] wszName;
    wszName = nullptr;
    wszData = nullptr;
  }
...
bool
Import_Section (bmf::INI::File::Section& section, wchar_t* buffer, int start, int end)
{
  int key = start;
  for (int k = key; k < end; k++) {
    if (k < end - 1 && buffer [k] == L'=') {
...
void
bmf::INI::File::import (std::wstring import_data)
{
...
std::wstring&
bmf::INI::File::Section::get_value (std::wstring key)
{
...
bool
bmf::INI::File::Section::contains_key (std::wstring key)
{
  for (std::multimap <std::wstring, std::wstring>::iterator it = pairs.begin ();
  it != pairs.end ();
    it++) {
...
void
bmf::INI::File::Section::add_key_value (std::wstring key, std::wstring value)
{
  pairs.insert (std::pair <std::wstring, std::wstring> (key, value));
}

bool
bmf::INI::File::contains_section (std::wstring section)
{
  return sections.find (section) != sections.end ();
}

bmf::INI::File::Section&
bmf::INI::File::get_section (std::wstring section)
{
  return sections [section];// sections.find (section);
}

void
bmf::INI::File::write (std::wstring fname)
{
  FILE*   fOut;
  errno_t ret;

  // Strip Read-Only
  BMF_SetNormalFileAttribs (fname);

  TRY_FILE_IO (_wfopen_s (&fOut, fname.c_str (), L"w,ccs=UTF-16LE"), fname.c_str (), ret);

  if (ret != 0 || fOut == 0) {
    //BMF_MessageBox (L"ERROR: Cannot open INI file for writing. Is it read-only?", fname.c_str (), MB_OK | MB_ICONSTOP);
    return;
  }
...
const std::map <std::wstring, bmf::INI::File::Section>&
bmf::INI::File::get_sections (void)
{
  return sections;
}